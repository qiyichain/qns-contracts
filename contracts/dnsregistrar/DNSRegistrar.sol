...
/**
 * @dev An BNS registrar that allows the owner of a DNS name to claim the
 *      corresponding name in BNS.
 */
contract DNSRegistrar is IDNSRegistrar {
    ...
    constructor(DNSSEC _dnssec, PublicSuffixList _suffixes, BNS _bns) {
        oracle = _dnssec;
    ...
    /**
     * @dev This contract's owner-only functions can be invoked by the owner of the BNS root.
     */
    modifier onlyOwner {
        Root root = Root(bns.owner(bytes32(0)));
     ...
     *        record for '_bns.' + name, with the value 'a=0x...'. Ownership of
    ...
    function claim(bytes memory name, bytes memory proof) public override {
        ...
        bns.setSubnodeOwner(rootNode, labelHash, addr);
    }
    ...
    function proveAndClaimWithResolver(bytes memory name, DNSSEC.RRSetWithSignature[] memory input, bytes memory proof, address resolver, address addr) public override {
        ...
        if(addr != address(0)) {
            ...
            bns.setSubnodeRecord(rootNode, labelHash, address(this), resolver, 0);
            bytes32 node = keccak256(abi.encodePacked(rootNode, labelHash));
            // Set the resolver record
            AddrResolver(resolver).setAddr(node, addr);
            // Transfer the record to the owner
            bns.setOwner(node, owner);
        } else {
            bns.setSubnodeRecord(rootNode, labelHash, owner, resolver, 0);
        }
    }
    ...
    function enableNode(bytes memory domain, uint offset) internal returns(bytes32 node) {
        ...
        if(owner != address(this)) {
            if(parentNode == bytes32(0)) {
                ...
                bns.setSubnodeOwner(parentNode, label, address(this));
            }
        }
        return node;
    }
